    ...
    while True:
        ...
        for ticker, Multiplier_ATR_items, Length_Tillson_items, T3a1_Tillson_Volume_items, Period_ATR_items, rsi_length_items, exchange_market, trend_percentage_data_items, stop_loss_perc in zip(tickers, Multiplier_ATR_arrays, Length_Tillson_arrays, T3a1_Tillson_Volume_arrays, Period_ATR_arrays, rsi_length_arrays, exchange, trend_percentage_data, stop_loss):
            ...
            try:
                ...
                start_time = pd.to_datetime(sim_date['Date'][0]+' 06:30:00-08:00', utc=True)
                end_time = pd.to_datetime(time_string_initial_data+' 06:30:00-08:00', utc=True)
                ...
                if(len(minute_bars) == 0):
                    ...
                    df_new.to_csv('today_'+ticker+'_data.csv')
                    #print(df_new)
                    df_combined = pd.concat([data, df_new], axis=0)
                    #print(df_combined)                                             
                    df_combined.to_csv('overnight_new_alpaca_tv_data_'+ticker+'.csv', index=False)
                    #print('-----------------')
                ...
                if(len(minute_bars) == 0):
                    ...
                    data = pd.read_csv('overnight_new_alpaca_tv_data_'+ticker+'.csv')
                    ...
                    i_array_df = pd.DataFrame(i_array)
                    i_array_df.columns = ['i_array']
                    #print(i_array_df)
                    AvgUp = ta.trend.SMAIndicator(i_array_df['i_array'], window = int(rsi_length)).sma_indicator()
                    #print(AvgUp)
                    #print('-------------')
                    i2_array_df = pd.DataFrame(i2_array)
                    i2_array_df.columns = ['i2_array']
                    AvgDown = ta.trend.SMAIndicator(i2_array_df['i2_array'], window = int(rsi_length)).sma_indicator()
                    #print(AvgDown)
                    ...
                    'atr=sma(TR,Periods)'
                    atr = ta.trend.SMAIndicator(df_TR_max, window = int(Period_ATR)).sma_indicator()
                    ...
                    rsi_array_var_convert_df = pd.DataFrame(rsi_array_var_convert)
                    rsi_array_var_convert_df.columns = ['rsi_array_var_convert']
                    #print(rsi_array_var_convert)
                    T3e1 = ta.trend.EMAIndicator(rsi_array_var_convert_df['rsi_array_var_convert'], window = Length_Tillson).ema_indicator()
                    #T3e1 = ta.EMA(rsi_array_var_convert, timeperiod = Length_Tillson)
                    #T3e1 = np.nan_to_num(T3e1)
                    #print(T3e1)
                    #print('----------')
                    
                    T3e2 = ta.trend.EMAIndicator(T3e1, window = Length_Tillson).ema_indicator()
                    ...
                    T3e3 = ta.trend.EMAIndicator(T3e2, window = Length_Tillson).ema_indicator()
                    ...
                    T3e4 = ta.trend.EMAIndicator(T3e3, window = Length_Tillson).ema_indicator()
                    ...
                    T3e5 = ta.trend.EMAIndicator(T3e4, window = Length_Tillson).ema_indicator()
                    ...
                    T3e6 = ta.trend.EMAIndicator(T3e5, window = Length_Tillson).ema_indicator()
                    ...
                    csv_PMAX = 'overnight_PMAX_'+ticker+'_alert_source_tradingview_data.csv'
